-- Product Management: master catalog + many-to-many Partner assignment.
--
-- products            — admin-managed catalog (Fire Fighting System, Fire Alarm
--                        System, Fire Pumps, Tanks, ...). Deactivate via is_active,
--                        never delete — partner_products / inquiry_items rows must
--                        keep referring to a real row for historical accuracy.
-- partner_products     — many-to-many: a product can be assigned to many Partners,
--                        a Partner can have many products. UNIQUE prevents double-assign.
-- inquiry_items.product_id — nullable, additive. Existing rows get NULL and are
--                        unaffected. It's for traceability only — the existing
--                        free-text type/system/system_type columns already capture
--                        what was picked at creation time, so a later edit or
--                        deactivation of the product never changes how a past
--                        inquiry item renders.
--
-- Enforcement: the Agent visit-creation flow writes inquiry_items directly from
-- the browser with the anon key (src/api/inquirySupabase.js), bypassing the
-- Express backend entirely — so "Agent can't pick a product not assigned to the
-- selected Partner" can only be reliably enforced with a DB trigger, not just a
-- backend-layer check. See enforce_inquiry_item_product_assignment() below.
...
-- ---------------------------------------------------------------------------
-- Storage bucket for product images (public read; admin uploads from the browser).
-- ---------------------------------------------------------------------------
